...
class InflowCalculatorTest {
  ...
  fun `test calculateInflows with empty snapshot list`() {
    ...
    assertEquals(BucketCreator.BUCKET_ARRAY_SIZE, inflows.length)
  ...
  fun `test calculateInflows with single block snapshots`() {
    val now = Instant.now()
    val buckets1 = F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 1000.0 }
    val buckets2 = F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 2000.0 }
  ...
  fun `test calculateInflows with consistent inflow rate`() {
    ...
    val buckets1 = F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 1_000_000.0 }
    val buckets2 = F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 2_000_000.0 }
    val buckets3 = F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 3_000_000.0 }
    ...
    assertEquals(2_000_000.0, inflows[BucketCreator.BUCKET_ARRAY_SIZE - 1])
  }
  ...
  fun `test calculateInflows with different rates per bucket`() {
    ...
    val buckets1 =
      F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { idx ->
        when (idx) {
          0 -> 1_000_000.0
          1 -> 2_000_000.0
          2 -> 3_000_000.0
  ...
  fun `test calculateInflows considers only first and last snapshot per block height`() {
    ...
    val snapshots = listOf(
      MempoolSnapshotF64Array(now, 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 1000.0 }),
      MempoolSnapshotF64Array(now.plusSeconds(100), 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 500.0 }), // Dip should be ignored
      MempoolSnapshotF64Array(now.plusSeconds(300), 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 2000.0 })
    )
  ...
  fun `test calculateInflows handles multiple block heights`() {
    ...
    val snapshots = listOf(
      // Block 100
      MempoolSnapshotF64Array(now, 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 1000.0 }),
      MempoolSnapshotF64Array(now.plusSeconds(100), 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 500.0 }), // Should be ignored
      MempoolSnapshotF64Array(now.plusSeconds(200), 100, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 2000.0 }),

      // Block 101
      MempoolSnapshotF64Array(now.plusSeconds(300), 101, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 2000.0 }),
      MempoolSnapshotF64Array(now.plusSeconds(400), 101, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 1500.0 }), // Should be ignored
      MempoolSnapshotF64Array(now.plusSeconds(500), 101, F64Array(BucketCreator.BUCKET_ARRAY_SIZE) { 3000.0 })
    )
